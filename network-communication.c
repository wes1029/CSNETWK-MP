...
void send_group_create(SOCKET sock, const char *group_id, const char *group_name, const char *members) {
    ...
    snprintf(msg, sizeof(msg),
        "TYPE: GROUP_CREATE\nFROM: %s\nGROUP_ID: %s\nGROUP_NAME: %s\nMEMBERS: %s\nTIMESTAMP: %ld\nTOKEN: %s\n\n",
        user_id, group_id, group_name, members, ts, token);

    // Send to all members
    char members_copy[512];
    strncpy(members_copy, members, sizeof(members_copy));
    char *tok = strtok(members_copy, ",");
    ...
    while (tok) {
        ...
        EnterCriticalSection(&peer_lock);
        for (int i = 0; i < peer_count; ++i) {
            if (strcmp(peer_list[i].user_id, tok) == 0) {
                addr = peer_list[i].address;
                found = peer_list[i].has_address;
                break;
            }
        }
    ...
    printf("Group \"%s\" created and invite sent.\n", group_name);
}
...
void update_group(const char *gid, const char *add_csv, const char *remove_csv) {
    for (int i = 0; i < group_count; ++i) {
        if (strcmp(group_list[i].group_id, gid) == 0) {
            ...
            if (is_group_member(gid, user_id)) {
                printf("The group \"%s\" member list was updated.\n", group_list[i].group_name);
            }
            return;
        }
    }
    ...
    while (1) {
        ...
        if (len > 0) {
            ...
            if (strstr(type_line, "TYPE: PING")) {
                ...
                char *from = NULL, *to = NULL, *content = NULL;
                char *line;
                ...
                */
                ...
                if (from && to && strcmp(to, user_id) == 0) {
                    ...
                    if (strstr(type_line, "TYPE: FOLLOW")) {
                        printf("User %s has followed you\n", display);
                    } else {
                        printf("User %s has unfollowed you\n", display);
                    }
                }
            } else if (strstr(type_line, "TYPE: POST")) {
                char *uid = NULL, *content = NULL, *line, *timestamp_str = NULL;
                long ts = 0;
                ...
                char *from = NULL, *to = NULL, *action = NULL;
                char *post_ts = NULL;
                char *line;
                ...
                if (to && strcmp(to, user_id) == 0 && from && post_ts && action) {
                    ...
                    printf("\n%s %s your post", from_disp, strcmp(action, "LIKE") == 0 ? "liked" : "unliked");
                    if (content) {
                        printf(": \"%s\"\n", content);
                    } else {
                        printf(".\n");
                ...
                char *from = NULL, *gid = NULL, *gname = NULL, *members = NULL, *ts = NULL, *line;
                while ((line = strtok(NULL, "\n")) != NULL) {
                    if (strncmp(line, "FROM: ", 6) == 0) from = line + 6;
                ...
                if (from && gid && gname && members && ts) {
                    add_group(from, gid, gname, members, atol(ts));
                }
            } else if (strstr(type_line, "TYPE: GROUP_UPDATE")) {
                char *from = NULL, *gid = NULL, *add = NULL, *remove = NULL, *line;
                while ((line = strtok(NULL, "\n")) != NULL) {
                    if (strncmp(line, "FROM: ", 6) == 0) from = line + 6;
                    else if (strncmp(line, "GROUP_ID: ", 10) == 0) gid = line + 10;
                ...
                char *from = NULL, *gid = NULL, *content = NULL, *line;
                while ((line = strtok(NULL, "\n")) != NULL) {
                    if (strncmp(line, "FROM: ", 6) == 0) from = line + 6;
                    else if (strncmp(line, "GROUP_ID: ", 10) == 0) gid = line + 10;
                    else if (strncmp(line, "CONTENT: ", 9) == 0) content = line + 9;
...
void list_peers(SOCKET sock) {
    ...
    // skip if user pressed Enter without number
    if (input[0] == '\n') {
        LeaveCriticalSection(&peer_lock);
        return;
    }
...
void input_loop(void *arg) {
    ...
    while (1) {
        ...
        if (cmd == 'L' || cmd == 'l') {
            ...
            if (subcmd == '1') {
                char gid[50], gname[50], members[512];
                printf("Enter group ID: ");
                fgets(gid, sizeof(gid), stdin); gid[strcspn(gid, "\n")] = 0;

                printf("Enter group name: ");
                fgets(gname, sizeof(gname), stdin); gname[strcspn(gname, "\n")] = 0;

                printf("Enter comma-separated user_ids to include (include yourself!): ");
                fgets(members, sizeof(members), stdin); members[strcspn(members, "\n")] = 0;

                send_group_create(sock, gid, gname, members);
            } else if (subcmd == '2') {
                char gid[50], add[512], remove[512];
                printf("Enter group ID: ");
                fgets(gid, sizeof(gid), stdin); gid[strcspn(gid, "\n")] = 0;

                printf("Enter comma-separated user_ids to ADD (or blank): ");
                fgets(add, sizeof(add), stdin); add[strcspn(add, "\n")] = 0;

                printf("Enter comma-separated user_ids to REMOVE (or blank): ");
                fgets(remove, sizeof(remove), stdin); remove[strcspn(remove, "\n")] = 0;

                send_group_update(sock, gid, add, remove);
            } else if (subcmd == '3') {
